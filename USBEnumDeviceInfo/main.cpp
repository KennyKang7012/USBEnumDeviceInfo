...
bool ChecktHardwareIDs(TCHAR *szVid, TCHAR *szPid, TCHAR *szRev, TCHAR *szMi)
{
	bool bResult = false;
	int nVid = 0, nPid = 0, nRev = 0, nMi = 0;
	TCHAR *strVid = TEXT("VID_0B05");
	TCHAR *strPid = TEXT("PID_1866");
	TCHAR *strRev = TEXT("REV_0002");
	TCHAR *strMi = TEXT("MI_00");

	_tprintf(TEXT("szVid: %s\n"), szVid);
	_tprintf(TEXT("szPid: %s\n"), szPid);
	_tprintf(TEXT("szRev: %s\n"), szRev);
	_tprintf(TEXT("szMi: %s\n"), szMi);

	nVid = wcscmp(szVid, strVid);
	nPid = wcscmp(szPid, strPid);
	nRev = wcscmp(szRev, strRev);
	nMi = wcscmp(szMi, strMi);
	_tprintf(TEXT("nVid: %d\n"), nVid);
	_tprintf(TEXT("nPid: %d\n"), nPid);
	_tprintf(TEXT("nRev: %d\n"), nRev);
	_tprintf(TEXT("nMi: %d\n"), nMi);

	if ((nVid == 0) && (nPid == 0) && (nRev == 0) && (nMi == 0)) {
		bResult = true;
		nVid = 0;
		nPid = 0;
		nRev = 0;
...
void GetHardwareIDs_Info(TCHAR *szHardwareIDs)
{
	bool bRes = false;
	unsigned j;
	LPTSTR pszToken, pszNextToken;
	const static LPCTSTR arPrefix[4] = { TEXT("VID_"), TEXT("PID_"), TEXT("REV_"), TEXT("MI_") };
	TCHAR szVid[MAX_DEVICE_ID_LEN], szPid[MAX_DEVICE_ID_LEN], szRev[MAX_DEVICE_ID_LEN], szMi[MAX_DEVICE_ID_LEN];
	TCHAR sVid[MAX_DEVICE_ID_LEN], sPid[MAX_DEVICE_ID_LEN], sRev[MAX_DEVICE_ID_LEN], sMi[MAX_DEVICE_ID_LEN];

	pszToken = _tcstok_s(szHardwareIDs, TEXT("\\#&"), &pszNextToken);
	while (pszToken != NULL) {
		szVid[0] = TEXT('\0');
		szPid[0] = TEXT('\0');
		szRev[0] = TEXT('\0');
		...
		if (szVid[0] != TEXT('\0')) {
			sVid[0] = TEXT('\0');
			_tprintf(TEXT("        vid: \"%s\"\n"), szVid);
			_tcscpy_s(sVid, ARRAY_SIZE(sVid), szVid);
		}
		if (szPid[0] != TEXT('\0')) {
			sPid[0] = TEXT('\0');
			_tprintf(TEXT("        pid: \"%s\"\n"), szPid);
			_tcscpy_s(sPid, ARRAY_SIZE(sPid), szPid);
		}
		if (szRev[0] != TEXT('\0')) {
			sRev[0] = TEXT('\0');
			_tprintf(TEXT("        rev: \"%s\"\n"), szRev);
			_tcscpy_s(sRev, ARRAY_SIZE(sRev), szRev);
		}
		if (szMi[0] != TEXT('\0')) {
			sMi[0] = TEXT('\0');
			_tprintf(TEXT("        mi: \"%s\"\n"), szMi);
			_tcscpy_s(sMi, ARRAY_SIZE(sMi), szMi);
		}
		pszToken = _tcstok_s(NULL, TEXT("\\#&"), &pszNextToken);
	}
	bRes = ChecktHardwareIDs(sVid, sPid, sRev, sMi);
	_tprintf(TEXT("        bRes: \"%d\"\n"), bRes);
}
...
void ListDevices(CONST GUID *pClassGuid, LPCTSTR pszEnumerator)
{
	...
	for (i = 0; ; i++) {
		...
		if (SetupDiGetDeviceRegistryProperty(hDevInfo, &DeviceInfoData, SPDRP_HARDWAREID,
			...
			_tprintf(TEXT("        ***************************************\n"));
			GetHardwareIDs_Info(szHardwareIDs);
			_tprintf(TEXT("        ***************************************\n"));
		}
